...
void ROCCurve(unsigned char *ImageMSF, unsigned char *ImageEndBranch, int MSFCols, int MSFRows)
{
    ...
    char find[2];
    unsigned char *temp;
    Rows1 = Cols1 = TP = FP = FN = TN = thresh = k = found = 0;
    ...
    temp = (unsigned char *)calloc(MSFRows * MSFCols, sizeof(unsigned char));
    ...
    for (i = 0; i <= 255; i++)
    {
		...
		while((fscanf(Groundfpt, "%s %d %d\n", letter, &Cols1, &Rows1)) != EOF)
		{
            ...
            if ((found == 1) && (strcmp(letter, find) == 0))
            {
                TP++;
            }
            if ((found == 1) && (strcmp(letter, find) != 0))
            ...
            if ((found == 0) && (strcmp(letter, find) == 0))
            {
                FN++;
            }
            if ((found == 0) && (strcmp(letter, find) != 0))
