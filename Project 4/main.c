...

{
	...
	case WM_LBUTTONDOWN:case WM_RBUTTONDOWN:
		...
		if (RegGrowStep == 1 || RegGrowPlay == 1)
		{
...
void preGrow()
{
	...
	c = XCoord;	// Set the column to use equal to the x coodrinate where the mouse was clicked
	...
	while (qt != qh)
	{
		...
		for (r2 = -3; r2 <= 3; r2++)
			for (c2 = -3; c2 <= 3; c2++)
			{
				...
				if ((abs((int)(image[(queue[qt] / COLS + r2) * COLS + queue[qt] % COLS + c2]) - average) > u_difference) || (distance > u_distance))
					continue;
				if (SetColor == 1)
				{
